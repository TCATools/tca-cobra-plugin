...
class CAST(object):
    ...
    def functions(self):
        """
        使用 Python 正则获取当前文件中的所有函数定义
        :return: 函数字典 {函数名: {'start': 起始行号, 'end': 结束行号}} 或 False
        """
        ...
        try:
            ...
            logger.critical('[AST] Failed to read file {0}: {1}'.format(self.file_path, e))
            return False
        ...
        for line_number, line_content in enumerate(file_lines, start=1):
            line_content = line_content.strip()
            if line_content == '':
                continue

            # 检查该行是否匹配函数正则
            if not re.search(regex_functions, line_content):
                continue

            # 跳过注释行
            annotation_match = re.findall(regex_annotation, line_content)
            if len(annotation_match) >= 1 and annotation_match[0] != '':
                logger.info("[AST] This function is annotation")

            # 提取函数名
            function_name = re.findall(regex_functions, line_content)
            if len(function_name) >= 1:
                ...
                # 更新上一个函数的结束行号
                if match_count > 0 and prev_function_name in functions:
                    functions[prev_function_name]['end'] = str(line_number)

                prev_function_name = function_name
                functions[function_name] = {
                    'start': str(line_number),
                    'end': None  # 下一个函数的起始行或文件末尾
                }
                match_count += 1
            else:
                logger.info("[AST] Can't get function name: {0}:{1}".format(line_number, line_content))
