...
class Version:
    """
    处理 version 中的一些特殊情况
    不再依赖 distutils.version.LooseVersion，自行实现版本解析与比较
    """
    ...
    def __repr__(self):
        return "Version('{}')" .format(self.vstring)
                ...
                with open(requirement, 'r') as f:
                    for line in f:
                        ...
                        try:
                            ...
                            logger.warning('[DEP] Failed to parse requirement: {} ({})'.format(line, e))
            ...
            if 'pom.xml' in pom:
                try:
                    ...
                    for child in childs:
                        ...
                        group_id = child_elements[0].text
                        artifact_id = child_elements[1].text
                        if len(child_elements) > 2:
                            version = child_elements[2].text
